 ...
 */
	...
	CI_Input = Object.create(Events.EventEmitter.prototype);
	
	CI_Input.parent = Events.EventEmitter.prototype;
	CI_Input.name = 'CI_Input';
	...
	*/
	CI_Input.__construct = function() {
		...
		this.$use_xss_clean	= (CI_Common.config_item('global_xss_filtering') === true) ? true : false;
		this.$allow_get_array	= (CI_Common.config_item('enable_query_strings') === true) ? true : false;
		this._sanitize_globals();
		
		return this;
	}
	
	CI_Input.__load = function(random) {
	...
	*/
	CI_Input._sanitize_globals = function() {
		...
		var $array = [PHP.$_GET, PHP.$_POST, PHP.$_COOKIE, PHP.$_SERVER, PHP.$_FILES, PHP.$_ENV, (PHP.$_SESSION && PHP.is_array(PHP.$_SESSION)) ? PHP.$_SESSION : {}];
		
		for(var $global in $array) {
			if ( ! PHP.is_array($array[$global])) {
				if ( ! PHP.in_array($array[$global], $protected)) {
					PHP.unset(PHP.$GLOBALS[$array[$global]]);
				}
			} else {
				for(var $key in $array[$global]) {
					if ( ! PHP.in_array($key, $protected)) {
						PHP.unset(PHP.$GLOBALS[$key]);
					}

					if (PHP.is_array($array[$global][$key])) {
						for(var $k in $array[$global][$key]) {
							if ( ! PHP.in_array($k, $protected)) {
								PHP.unset(PHP.$GLOBALS[$k]);
							}
						}
					}
				}
	...
	*/
	CI_Input._clean_input_keys = function($str) {
		if ( ! PHP.preg_match(new RegExp("^[a-z0-9:_\/-]+$", "i"), $str)) {
			PHP.exit('Disallowed Key Characters.', 500);
		}
	...
	*/
	CI_Input.ip_address = function() {
		...
		if (CI_Common.config_item('proxy_ips') != '' && this.server('HTTP_X_FORWARDED_FOR') && this.server('REMOTE_ADDR')) {
			var $proxies = PHP.preg_split('/[\s,]/', CI_Common.config_item('proxy_ips'), -1, PHP.flag.PREG_SPLIT_NO_EMPTY);
			$proxies = PHP.is_array($proxies) ? $proxies : [];

			this.$ip_address = PHP.in_array(PHP.$_SERVER['REMOTE_ADDR'], $proxies) ? PHP.$_SERVER['HTTP_X_FORWARDED_FOR'] : PHP.$_SERVER['REMOTE_ADDR'];
		} else if (this.server('REMOTE_ADDR') && this.server('HTTP_CLIENT_IP')) {
			this.$ip_address = PHP.$_SERVER['HTTP_CLIENT_IP'];
		} else if (this.server('REMOTE_ADDR')) {
			this.$ip_address = PHP.$_SERVER['REMOTE_ADDR'];
		} else if (this.server('HTTP_CLIENT_IP')) {
			this.$ip_address = PHP.$_SERVER['HTTP_CLIENT_IP'];
		} else if (this.server('HTTP_X_FORWARDED_FOR')) {
			this.$ip_address = PHP.$_SERVER['HTTP_X_FORWARDED_FOR'];
		}
		...
		if(PHP.strstr(this.$ip_address, ',')) {
			var $x = PHP.explode(',', this.$ip_address);
			this.$ip_address = PHP.trim(PHP.end($x));
		}
	...
	*/
	CI_Input.valid_ip = function($ip) {
		...
		// Check each segment
		for(var $segment in $ip_segments) {
			// IP segments must be digits and can not be 
			// longer than 3 digits or greater then 255
			if ($segment == '' || PHP.preg_match(new RegExp("[^0-9]"), $segment) || $segment > 255 || PHP.strlen($segment) > 3) {
				return false;
			}
		}
	...
	*/
	CI_Input.user_agent = function() {
		...
		this.$user_agent = ( ! PHP.$_SERVER['HTTP_USER_AGENT']) ? false : PHP.$_SERVER['HTTP_USER_AGENT'];
	...
	*/
	CI_Input.xss_clean = function($str, $is_image) {
		...
		*/
		...
		$str = $str.replace(new RegExp('\&([a-z\_0-9]+)\=([a-z\_0-9]+)', 'i'), this.xss_hash() + "$1=$2");
		...
		*
		*/
		$str = $str.replace(new RegExp('(&\#?[0-9a-z]{2,})([\x00-\x20])*;?', 'i'), "$1;$2");
		...
		*
		*/
		$str = $str.replace(new RegExp('&\#x?)([0-9A-F]+);?', 'i'), "$1$2;");
		...
		*/
		...
		for(var $key in this.$never_allowed_str) {
			$str = $str.replace($key, this.$never_allowed_str[$key]);   
		}

		for(var $key in this.$never_allowed_regex) {
			$str = $str.replace(new RegExp($key, 'i'), this.$never_allowed_regex[$key]);   
		}
		...
		*/
		if ($is_image === true) {
			...
			$str = $str.replace(new RegExp('<\?(php)', 'i'), "&lt;?$1");
		} else {
		...
		*
		*/
		var $words = ['javascript', 'expression', 'vbscript', 'script', 'applet', 'alert', 'document', 'write', 'cookie', 'window'];
		
		for(var $word in $words) {
			var $temp = '';

			for ($i = 0, $wordlen = strlen($word); $i < $wordlen; $i++) {
				$temp += PHP.substr($words[$word], $i, 1) + "\s*";
			}

			// We only want to do this when it is followed by a non-word character
			// That way valid stuff like "dealer to" does not become "dealerto"
			$str = PHP.preg_replace_callback(new RegExp('(' + PHP.substr($temp, 0, -3) + ')(\W)', 'is'), [this, '_compact_exploded_words'], $str);
		}
		...
		*/
		do {
			...
			if (PHP.preg_match(new RegExp("<a", "i"), $str)) {
				$str = PHP.preg_replace_callback("/<a\s+([^>]*?)(>|$)/si", [this, '_js_link_removal'], $str);
			}

			if (PHP.preg_match(new RegExp("<img", "i"), $str)) {
				$str = PHP.preg_replace_callback("/<img\s+([^>]*?)(\s?/?>|$/si", [this, '_js_img_removal'], $str);
			}

			if (PHP.preg_match(new RegExp("script", "i"), $str) || PHP.preg_match(new RegExp("xss", "i"), $str)) {
				$str = $str.replace(new RegExp("<(/*)(script|xss)(.*?)\>", "si"), '[removed]');
			}
		}
		while($original != $str);
		...
		*
		*/
		var $event_handlers = ['[^a-z_\-]on\w*','xmlns'];
		...
		$str = $str.replace(new RegExp("<([^><]+?)(" + PHP.implode('|', $event_handlers) + ")(\s*=\s*[^><]*)([><]*)", 'i'), "<$1$4");
		...
		*/
		...
		$str = PHP.preg_replace_callback(new RegExp('<(/*\s*)(' + $naughty + ')([^><]*)([><]*)', 'is'), [this, '_sanitize_naughty_html'], $str);
		...
		*
		*/
		$str = $str.replace(new RegExp('(alert|cmd|passthru|eval|exec|expression|system|fopen|fsockopen|file|file_get_contents|readfile|unlink)(\s*)\((.*?)\)', 'si'), "$1$2&#40;$3&#41;");
		...
		*/
		for(var $key in this.$never_allowed_str) {
			$str = $str.replace(new RegExp($key), this.$never_allowed_str[$key]);   
		}
	...
	*/
	CI_Input._remove_invisible_characters = function($str) {
		...
		do {
			var $cleaned = $str;
			$str = PHP.str_replace($non_displayables, '', $str);
		}
		while ($cleaned != $str);
	...
	*/
	CI_Input._compact_exploded_words = function($matches) {
		return $matches[1].replace(new RegExp('\s+', 's'), '') + $matches[2];
	}
	...
	*/
	CI_Input._js_link_removal = function($match) {
		...
		return PHP.str_replace($match[1], $attributes.replace(new RegExp("href=.*?(alert\(|alert&\#40;|javascript\:|charset\=|window\.|document\.|\.cookie|<script|<xss|base64\s*,)", 'si'), ''), $match[0]);
	}
	...
	*/
	CI_Input._js_img_removal = function($match) {
		var $attributes = this._filter_attributes(PHP.str_replace(['<', '>'], '', $match[1]));
		return PHP.str_replace($match[1], $attributes.replace(new RegExp("src=.*?(alert\(|alert&\#40;|javascript\:|charset\=|window\.|document\.|\.cookie|<script|<xss|base64\s*,)", 'si'), ''), $match[0]);
	}
	...
	*/
	CI_Input._html_entity_decode = function($str, $charset) {
		...
		if (PHP.function_exists('html_entity_decode') && (PHP.strtolower($charset) != 'utf-8' || PHP.version_compare(PHP.phpversion(), '5.0.0', '>='))) {
			...
			$str = $str.replace(new RegExp('&#x(0*[0-9a-f]{2,5})', 'ei'), 'chr(hexdec("$1"))');
			return $str.replace(new RegExp('&#([0-9]{2,4})', 'e'), 'chr($1)');
		}

		// Numeric Entities
		$str = $str.replace(new RegExp('&#x(0*[0-9a-f]{2,5});{0,1}', 'ei'), 'chr(hexdec("$1"))');
		$str = $str.replace(new RegExp('&#([0-9]{2,4});{0,1}', 'e'), 'chr($1)');
	...
	*/
	CI_Input._filter_attributes = function($str) {
		...
		if (PHP.preg_match_all(new RegExp('\s*[a-z\-]+\s*=\s*(\042|\047)([^$1]*?)$1', 'is'), $str, $matches)) {
			for(var $match in $matches[0]) {
				$out += $matches[0][$match].replace(new RegExp('\*.*?\*', 's'), '');
			}
		}
