 ...
 */
	...
	DB.__construct = function($params, $active_record_override) {
		$params = $params || '';
		$active_record_override = $active_record_override || false;
	
		// Load the DB config file if a DSN string wasn't passed
		if (PHP.is_string($params) && PHP.strpos($params, '://') === false) {
			if (PHP.file_exists(PHP.constant('APPPATH') + 'config/database' + PHP.constant('EXT'))) {
				var $database = require(PHP.constant('APPPATH') + 'config/database' + PHP.constant('EXT'));
			} else {
				var $database = require(PHP.constant('BASEPATH') + 'config/database'  + PHP.constant('EXT'));
			}
			...
			if($params != '') {
				var $active_group = $params;
			} else {
				var $active_group = $database.active_group;
			}
		...
		if ($active_record_override == true) {
			...
			var $active_record = $database.active_record;
		}
		...
		if($db.$autoinit == true) {
			MY_Loader.on('autoload', function(load) {
